...
class Wrapper:
    ...
    def setTimeout(self, timeout: float):
        self.lastTime = datetime.now() #datetime.now(timezone.utc)
    ...
    def historicalDataUpdate(self, reqId: int, bar: BarData):
        ...
        bars.updateEvent.emit(bars, hasNewBar, self.ib)
    ...
    def tcpDataProcessed(self):
        ...
        if self.pendingTickers:
            for ticker in self.pendingTickers:
                ticker.time = datetime.now() #self.lastTime
                ticker.updateEvent.emit(ticker)
            self.ib.pendingTickersEvent.emit(self.pendingTickers, self.ib)
